  ...
  implements IPaymentModuleService
{
  ...
  async createPaymentSession(
    ...
    const balance = MathBN.sub(
      paymentCollection.raw_amount,
      paymentCollection.raw_authorized_amount ?? 0,
      MathBN.mult(paymentCollection.raw_refunded_amount ?? 0, -1)
    )
    ...
    if (MathBN.lte(balance, 0) || MathBN.gt(amount, balance)) {
      throw new MedusaError(
        MedusaError.Types.INVALID_DATA,
        `The payment amount exceeds the available balance amount to be paid.`
      )
    }
